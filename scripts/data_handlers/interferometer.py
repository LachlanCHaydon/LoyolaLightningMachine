...
from utils.ta_tools import compute_true_angles, angles_to_cosines
...
class InterferometerHandler:
    ...
    def run_lma_calibration(self, lma_filepath: str, intf_t0_sod: float,
                            ...
                            chi2_max: float = 2.0,
                            alt_min_m: float = 1000.0,
                            alt_max_m: float = 15000.0,
                            power_min_dbw: float = -20.0,
                            sigma_clip_thresh: float = 2.0,
                            sigma_clip_n: int = 3) -> Optional[Dict]:
        """
        Derive cosShiftA/B by cross-matching loaded raw INTF data with LMA sources.

        Parameters
        ----------
        lma_filepath : str
            Path to LMA data file (comment lines begin with '#').
        intf_t0_sod : float
            Absolute seconds-of-day corresponding to raw_time == 0 ms.
            Typically:  event_SOD + trigger_offset_s
        match_window_us : float
            Temporal matching window in microseconds.
        chi2_max : float
            LMA quality filter: maximum chi-squared.
        alt_min_m : float
            LMA quality filter: minimum altitude (metres).
        alt_max_m : float
            LMA quality filter: maximum altitude (metres).
        power_min_dbw : float
            LMA quality filter: minimum source power (dBW).
        sigma_clip_thresh : float
            Rejection threshold (sigma) for outlier clipping.
        sigma_clip_n : int
        ...
        -------
        dict with keys:
            cos_shift_a, cos_shift_b, se_a, se_b,
            n_matched, n_final, formal_error_deg, pairs
        or None on failure.
        """
        if not self.is_loaded:
            return None
        ...
        try:
            # Load LMA — variable comment-header format, columns 0-5
            lma_data = np.genfromtxt(lma_filepath, comments='#',
                                     usecols=(0, 1, 2, 3, 4, 5))
            if lma_data.ndim == 1:
                lma_data = lma_data.reshape(1, -1)
            if len(lma_data) == 0:
                return None

            # Quality filter
            mask = ((lma_data[:, 4] <= chi2_max) &
                    (lma_data[:, 3] >= alt_min_m) &
                    (lma_data[:, 3] <= alt_max_m) &
                    (lma_data[:, 5] >= power_min_dbw))
            lma = {
                'sod':   lma_data[mask, 0],
                'lat':   lma_data[mask, 1],
                'lon':   lma_data[mask, 2],
                'alt':   lma_data[mask, 3],
                'chi2':  lma_data[mask, 4],
                'power': lma_data[mask, 5],
            }
            if len(lma['sod']) == 0:
                return None

            # Convert raw INTF time (ms from file start) → absolute SOD
            intf_sod = intf_t0_sod + self.raw_time / 1000.0

            # Time matching: for each LMA source find best INTF point within window
            window_sod = match_window_us / 1e6
            matched_intf_idx = []
            matched_lma_idx  = []
            shift_a_list = []
            shift_b_list = []

            for i_lma in range(len(lma['sod'])):
                dt = intf_sod - lma['sod'][i_lma]
                within = np.abs(dt) <= window_sod
                if not within.any():
                    continue
                best = np.where(within)[0][np.argmin(np.abs(dt[within]))]
                true_azi, true_elv, _ = compute_true_angles(
                    lma['lat'][i_lma], lma['lon'][i_lma], lma['alt'][i_lma])
                cA_true, cB_true = angles_to_cosines(true_azi, true_elv)
                shift_a_list.append(cA_true - self.cos_a[best])
                shift_b_list.append(cB_true - self.cos_b[best])
                matched_intf_idx.append(best)
            ...
            n_matched = len(shift_a)

            # Iterative sigma clipping
            keep = np.ones(n_matched, dtype=bool)
            for _ in range(sigma_clip_n):
                if keep.sum() < 3:
                    break
                med_a = np.median(shift_a[keep])
                med_b = np.median(shift_b[keep])
                std_a = np.std(shift_a[keep])
                std_b = np.std(shift_b[keep])
                if std_a == 0 or std_b == 0:
                    break
                keep = ((np.abs(shift_a - med_a) / std_a <= sigma_clip_thresh) &
                        (np.abs(shift_b - med_b) / std_b <= sigma_clip_thresh))

            n_final = int(keep.sum())
            if n_final == 0:
                return None

            cos_shift_a = float(np.median(shift_a[keep]))
            cos_shift_b = float(np.median(shift_b[keep]))
            std_a_f = float(np.std(shift_a[keep]))
            std_b_f = float(np.std(shift_b[keep]))
            # Standard error of the median (normal approximation: SE_med ≈ 1.2533 * std / sqrt(N))
            se_a = float(1.2533 * std_a_f / np.sqrt(n_final))
            se_b = float(1.2533 * std_b_f / np.sqrt(n_final))
            formal_err = float(np.sqrt(
                np.degrees(np.arcsin(min(abs(se_a), 1.0)))**2 +
                np.degrees(np.arcsin(min(abs(se_b), 1.0)))**2
            ))

            # Build per-pair detail list for display
            pairs = []
            for k in range(n_matched):
                i_lma  = matched_lma_idx[k]
                i_intf = matched_intf_idx[k]
                t_azi, t_elv, _ = compute_true_angles(
                    lma['lat'][i_lma], lma['lon'][i_lma], lma['alt'][i_lma])
                pairs.append({
                    'lma_sod':    float(lma['sod'][i_lma]),
                    'lma_lat':    float(lma['lat'][i_lma]),
                    'lma_lon':    float(lma['lon'][i_lma]),
                    'lma_alt_m':  float(lma['alt'][i_lma]),
                    'true_azi':   float(t_azi),
                    'true_elv':   float(t_elv),
                    'meas_cosA':  float(self.cos_a[i_intf]),
                    'meas_cosB':  float(self.cos_b[i_intf]),
                    'shift_a':    float(shift_a[k]),
                    'shift_b':    float(shift_b[k]),
                    'kept':       bool(keep[k]),
                })

            return {
                'cos_shift_a':      cos_shift_a,
                'cos_shift_b':      cos_shift_b,
                'se_a':             se_a,
                'se_b':             se_b,
                'n_matched':        n_matched,
                'n_final':          n_final,
            ...
            print(f"Error in LMA calibration: {e}")
            return None
