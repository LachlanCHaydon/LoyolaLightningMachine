...
class HomePlotterTab(ttk.Frame):
    ...
    def __init__(self, parent, main_app):
        ...
        # 2. Initialize UI Variables (State)
        # File paths
        self.file_paths = {
            'fa': tk.StringVar(),
            'intf': tk.StringVar(),
        ...
        self.time_markers = []
        self.marker_time_var = tk.StringVar()
        self.marker_label_var = tk.StringVar()
        self.marker_color_var = tk.StringVar(value="black")

        # 3. Build UI (Now safe to call)
        self._build_ui()
    ...
    def _build_markers_section(self):
        """Build time markers section."""
        frame = ttk.LabelFrame(self.scrollable_frame, text="Time Markers", padding=5)
        ...
        ttk.Label(input_row, text="Time (µs):").pack(side=tk.LEFT)
        ttk.Entry(input_row, textvariable=self.marker_time_var, width=10).pack(side=tk.LEFT, padx=2)

        ttk.Label(input_row, text="Label:").pack(side=tk.LEFT)
        ttk.Entry(input_row, textvariable=self.marker_label_var, width=12).pack(side=tk.LEFT, padx=2)
        ...
        color_combo = ttk.Combobox(color_row, textvariable=self.marker_color_var, width=10,
                                   values=['black', 'red', 'blue', 'green', 'orange', 'purple', 'gray'])
        color_combo.pack(side=tk.LEFT, padx=2)

        ttk.Button(color_row, text="Add Marker", command=self._add_time_marker).pack(side=tk.LEFT, padx=5)

        # Markers listbox
        ttk.Label(frame, text="Current Markers:").pack(anchor='w', pady=(5, 0))

        list_frame = ttk.Frame(frame)
        list_frame.pack(fill=tk.X, pady=2)

        self.markers_listbox = tk.Listbox(list_frame, height=4, exportselection=False)
        markers_scrollbar = ttk.Scrollbar(list_frame, orient='vertical', command=self.markers_listbox.yview)
        self.markers_listbox.configure(yscrollcommand=markers_scrollbar.set)
        self.markers_listbox.pack(side=tk.LEFT, fill=tk.X, expand=True)
        markers_scrollbar.pack(side=tk.RIGHT, fill='y')

        # Remove button
        ttk.Button(frame, text="Remove Selected", command=self._remove_time_marker).pack(fill=tk.X, pady=2)

    def _add_time_marker(self):
        """Add a time marker."""
        try:
            time_val = float(self.marker_time_var.get())
        except ValueError:
            messagebox.showerror("Error", "Invalid time value")
            return

        label = self.marker_label_var.get() or f"t={time_val}"
        color = self.marker_color_var.get() or "black"

        marker = {'time': time_val, 'label': label, 'color': color}
        self.time_markers.append(marker)

        # Update listbox
        self._update_markers_listbox()

        # Clear inputs
        self.marker_time_var.set("")
        self.marker_label_var.set("")

        # Update plot
        self._update_plot()

    def _remove_time_marker(self):
        """Remove selected time marker."""
        selection = self.markers_listbox.curselection()
        if not selection:
            return

        del self.time_markers[selection[0]]
        self._update_markers_listbox()
        self._update_plot()

    def _update_markers_listbox(self):
        """Update the markers listbox."""
        self.markers_listbox.delete(0, tk.END)
        for marker in self.time_markers:
            self.markers_listbox.insert(tk.END, f"{marker['time']} µs - {marker['label']} ({marker['color']})")
    ...
    def _update_plot(self):
        ...
        for ax_idx, ax in enumerate(axes_list):
            ...
            # Add zoom connector lines on main plot (solid black lines connecting to zoom subplot)
            if ax_idx == 0 and show_zoom:
                # Get current y-limits for full-height lines
                y_min, y_max = ax.get_ylim()

                # Draw solid black lines at zoom boundaries
                ax.axvline(x=zoom_start, color='black', linewidth=1.5, linestyle='-', zorder=10)
                ax.axvline(x=zoom_stop, color='black', linewidth=1.5, linestyle='-', zorder=10)

            # Plot time markers on all axes
            for marker in self.time_markers:
                if t_min <= marker['time'] <= t_max:
                    # Draw vertical line
                    line = ax.axvline(x=marker['time'], color=marker['color'],
                                      linewidth=1.5, linestyle='--', zorder=5)

                    # Add text label at top of plot
                    y_min, y_max = ax.get_ylim()
                    ax.text(marker['time'], y_max * 0.95, marker['label'],
                            rotation=90, verticalalignment='top', horizontalalignment='right',
                            fontsize=8, color=marker['color'], zorder=6)

                    # Add to legend only on main plot
                    if ax_idx == 0:
                        legend_handles.append(line)
                        legend_labels.append(marker['label'])
    ...
    def _save_to_project(self):
        ...
        state.home_plotter['time_markers'] = self.time_markers.copy()
        # Mark as unsaved
    ...
    def load_from_project(self):
        ...
        if hasattr(state, 'home_plotter') and state.home_plotter:
            self.time_markers = state.home_plotter.get('time_markers', [])
            self._update_markers_listbox()