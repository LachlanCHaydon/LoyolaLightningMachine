...
class MapVisualizerTab(ttk.Frame):
    ...
    def _on_row_select(self, event):
        ...
        if self.filtered_data is not None and self.selected_index < len(self.filtered_data):
            ...
            # Highlight on map if showing all points
    ...
    def _draw_base_map(self):
        ...
        if self.detector_coords is not None and len(self.detector_coords) > 0:
            lons = self.detector_coords[:, 2]
            lats = self.detector_coords[:, 1]
            self.ax.scatter(lons, lats, s=8, color='black', zorder=1)

            # Set axis limits based on detector array (with some padding)
            lon_pad = 0.02
            lat_pad = 0.02
            min_lon = min(np.min(lons), self.CAMERA_LON) - lon_pad
            max_lon = max(np.max(lons), self.CAMERA_LON) + lon_pad
            min_lat = min(np.min(lats), self.CAMERA_LAT) - lat_pad
            max_lat = max(np.max(lats), self.CAMERA_LAT) + lat_pad
    ...
    def _highlight_selected(self):
        """Highlight the selected strike on the map without redrawing everything."""
        ...
        # Plot larger version of the same marker (highlighted)
