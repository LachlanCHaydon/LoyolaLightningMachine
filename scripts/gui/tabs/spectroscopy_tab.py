...
class SpectroscopyTab(ttk.Frame):
    ...
    def _on_subtab_changed(self, event):
        """Handle sub-tab changes to refresh displays."""
        selected_tab = self.sub_notebook.select()
        tab_name = self.sub_notebook.tab(selected_tab, "text")

        if tab_name == "Export":
            self.export_slider.config(to=len(self.frames) - 1 if self.frames else 0)
            self._update_export_nav_label()
            self._update_export_summary()
            self._update_export_preview()
        elif tab_name == "ROI Tracking":
    ...
    def _build_export_tab(self):
        print("=== BUILDING EXPORT TAB ===")  # Add this line at the very start
        ...
        # Initialize variables first
        self.export_mode_var = tk.StringVar(value="single")
        self.export_frame_var = tk.StringVar(value="0")
        self.export_start_var = tk.StringVar(value="0")
        self.export_end_var = tk.StringVar(value="0")
        self.export_full_frame_var = tk.BooleanVar(value=True)
        self.export_roi_var = tk.BooleanVar(value=True)
        self.export_spectrum_var = tk.BooleanVar(value=True)
        self.export_show_roi_box_var = tk.BooleanVar(value=True)
        self.export_gridlines_var = tk.BooleanVar(value=True)
        self.camera_label_var = tk.StringVar(value="v711")
        self.camera_custom_var = tk.StringVar(value="")

        main_frame = ttk.Frame(self.export_frame)
        main_frame.pack(fill='both', expand=True, padx=10, pady=10)

        # Left panel - controls (width=670)
        left_container = ttk.Frame(main_frame, width=670)
        left_container.pack(side='left', fill='y', padx=5, pady=5)
        left_container.pack_propagate(False)

        # --- Summary ---
        summary_frame = ttk.LabelFrame(left_container, text="Analysis Summary")
        ...
        # --- Frame Selection ---
        select_frame = ttk.LabelFrame(left_container, text="Frame Selection")
        select_frame.pack(fill='x', pady=5, padx=5)

        # Single frame
        single_row = ttk.Frame(select_frame)
        single_row.pack(fill='x', padx=5, pady=2)
        ttk.Radiobutton(single_row, text="Current Frame:", value="single",
                        variable=self.export_mode_var).pack(side='left')
        ttk.Entry(single_row, textvariable=self.export_frame_var, width=6).pack(side='left', padx=5)
        ttk.Button(single_row, text="Use Current",
                   command=lambda: self.export_frame_var.set(str(self.current_frame_idx))).pack(side='left')

        # Range of frames
        range_row = ttk.Frame(select_frame)
        range_row.pack(fill='x', padx=5, pady=2)
        ttk.Radiobutton(range_row, text="Frame Range:", value="range",
                        variable=self.export_mode_var).pack(side='left')
        ttk.Entry(range_row, textvariable=self.export_start_var, width=6).pack(side='left', padx=2)
        ttk.Label(range_row, text="to").pack(side='left', padx=2)
        ttk.Entry(range_row, textvariable=self.export_end_var, width=6).pack(side='left', padx=2)

        # All frames
        all_row = ttk.Frame(select_frame)
        all_row.pack(fill='x', padx=5, pady=2)
        ttk.Radiobutton(all_row, text="All Frames", value="all",
                        variable=self.export_mode_var).pack(side='left')

        # --- Frame Navigation ---
        nav_frame = ttk.LabelFrame(left_container, text="Frame Navigation")
        nav_frame.pack(fill='x', pady=5, padx=5)

        nav_buttons = ttk.Frame(nav_frame)
        nav_buttons.pack(fill='x', padx=5, pady=2)
        ttk.Button(nav_buttons, text="← Prev",
                   command=self._export_prev_frame).pack(side='left', expand=True, fill='x')
        ttk.Button(nav_buttons, text="Next →",
                   command=self._export_next_frame).pack(side='left', expand=True, fill='x')


        self.export_slider = ttk.Scale(nav_frame, from_=0, to=0, orient='horizontal',
                                       command=self._on_export_slider)
        self.export_slider.pack(fill='x', padx=5, pady=2)

        self.export_nav_label = ttk.Label(nav_frame, text="Frame 0 / 0", font=('', 10, 'bold'))
        self.export_nav_label.pack(padx=5, pady=2)

        def _export_prev_frame(self):
            """Previous frame in export tab."""
            if self.current_frame_idx > 0:
                self.current_frame_idx -= 1
                self.export_slider.set(self.current_frame_idx)
                self.export_frame_var.set(str(self.current_frame_idx))
                self._update_export_nav_label()
                self._update_export_preview()

        def _export_next_frame(self):
            """Next frame in export tab."""
            if self.current_frame_idx < len(self.frames) - 1:
                self.current_frame_idx += 1
                self.export_slider.set(self.current_frame_idx)
                self.export_frame_var.set(str(self.current_frame_idx))
        ...
        def _on_export_slider(self, value):
            """Handle export slider change."""
            self.current_frame_idx = int(float(value))
            self.export_frame_var.set(str(self.current_frame_idx))
            self._update_export_nav_label()
            self._update_export_preview()

        def _update_export_nav_label(self):
            """Update export navigation label."""
            if not self.frames:
                self.export_nav_label.config(text="Frame 0 / 0")
                return
            frame = self.frames[self.current_frame_idx]
            self.export_nav_label.config(
                text=f"Frame {self.current_frame_idx} / {len(self.frames) - 1} - {frame.timestamp} μs"
            )
        # --- Export Options --- THIS WAS MISSING THE CHECKBOXES
        options_frame = ttk.LabelFrame(left_container, text="Export Options")
        options_frame.pack(fill='x', pady=5, padx=5)

        ttk.Checkbutton(options_frame, text="Export Full Frame",
                        variable=self.export_full_frame_var,
                        command=self._update_export_preview).pack(anchor='w', padx=5, pady=2)
        ttk.Checkbutton(options_frame, text="Export ROI Zoom",
                        variable=self.export_roi_var,
                        command=self._update_export_preview).pack(anchor='w', padx=5, pady=2)
        ttk.Checkbutton(options_frame, text="Show ROI box on full frame",
                        variable=self.export_show_roi_box_var,
                        command=self._update_export_preview).pack(anchor='w', padx=20, pady=2)
        ttk.Checkbutton(options_frame, text="Export Spectrum Plot",
                        variable=self.export_spectrum_var,
                        command=self._update_export_preview).pack(anchor='w', padx=5, pady=2)
        ttk.Checkbutton(options_frame, text="Show gridlines on spectrum",
                        variable=self.export_gridlines_var,
                        command=self._update_export_preview).pack(anchor='w', padx=20, pady=2)

        # --- Camera Label ---
        camera_frame = ttk.LabelFrame(left_container, text="Camera Label")
        camera_frame.pack(fill='x', pady=5, padx=5)

        camera_row = ttk.Frame(camera_frame)
        camera_row.pack(fill='x', padx=5, pady=2)
        ttk.Radiobutton(camera_row, text="v711", value="v711",
                        variable=self.camera_label_var).pack(side='left', padx=5)
        ttk.Radiobutton(camera_row, text="v2012", value="v2012",
                        variable=self.camera_label_var).pack(side='left', padx=5)
        ttk.Radiobutton(camera_row, text="Custom:", value="custom",
                        variable=self.camera_label_var).pack(side='left', padx=5)
        ttk.Entry(camera_row, textvariable=self.camera_custom_var, width=12).pack(side='left', padx=2)

        # --- Export Buttons ---
        button_frame = ttk.LabelFrame(left_container, text="Export")
        button_frame.pack(fill='x', pady=5, padx=5)

        ttk.Button(button_frame, text="Export Selected Frames",
                   command=self._export_selected_frames).pack(fill='x', padx=10, pady=5)

        ttk.Separator(button_frame, orient='horizontal').pack(fill='x', padx=5, pady=10)

        ttk.Label(button_frame, text="Data Export:", font=('', 9, 'bold')).pack(anchor='w', padx=5)

        ttk.Button(button_frame, text="Export Peak Data (CSV)",
                   command=self._export_csv).pack(fill='x', padx=10, pady=2)

        ttk.Button(button_frame, text="Export Calibration (JSON)",
                   command=self._export_calibration).pack(fill='x', padx=10, pady=2)

        ttk.Button(button_frame, text="Save to Project",
                   command=self._save_and_notify).pack(fill='x', padx=10, pady=5)
        tk.Button(button_frame, text="Export 777nm Time Series",
                  command=self._export_777nm_timeseries).pack(fill='x', padx=10, pady=2)

        # --- Right panel - Preview ---
        right_frame = ttk.Frame(main_frame)
        right_frame.pack(side='right', fill='both', expand=True, padx=5, pady=5)

        ttk.Label(right_frame, text="Export Preview", font=('', 10, 'bold')).pack(anchor='w', padx=5)

        self.export_fig = plt.Figure(figsize=(10, 8), dpi=100)
        self.export_canvas = FigureCanvasTkAgg(self.export_fig, master=right_frame)
        self.export_canvas.get_tk_widget().pack(fill='both', expand=True)

        ttk.Button(right_frame, text="Refresh Preview",
                   command=self._update_export_preview).pack(fill='x', padx=5, pady=5)
    ...
    def _get_export_mode_var(self):
        """Get or create the export mode variable."""
        if not hasattr(self, 'export_mode_var'):
            self.export_mode_var = tk.StringVar(value="single")
        return self.export_mode_var

    def _get_camera_label(self):
        """Get the camera label for titles."""
        if self.camera_label_var.get() == "custom":
            return self.camera_custom_var.get() or "Frame"
        return self.camera_label_var.get()

    def _get_frames_to_export(self):
        """Get list of frame indices to export based on selection."""
        mode = self.export_mode_var.get()

        if mode == "single":
            try:
                idx = int(self.export_frame_var.get())
            ...
            try:
                start = int(self.export_start_var.get())
                end = int(self.export_end_var.get())
                start = max(0, min(start, len(self.frames) - 1))
                end = max(0, min(end, len(self.frames) - 1))
                if start > end:
                    start, end = end, start
    ...
    def _update_export_preview(self):
        """Update the export preview showing what will be exported."""
        try:
            if not self.frames:
                return

            frame_indices = self._get_frames_to_export()
            ...
            effective_roi = self.roi.copy()
            effective_roi['y'] += frame.roi_offset_y
            camera_label = self._get_camera_label()

            # Clear figure
            self.export_fig.clf()

            # Determine what to show
            show_full = self.export_full_frame_var.get()
            show_roi = self.export_roi_var.get()
            show_spectrum = self.export_spectrum_var.get()

            if not (show_full or show_roi or show_spectrum):
                ax = self.export_fig.add_subplot(111)
                ax.text(0.5, 0.5, "Select at least one export option",
                        ha='center', va='center', fontsize=12)
                ax.axis('off')
                self.export_canvas.draw_idle()
                return

            # Use 2x2 grid like Review tab
            gs = gridspec.GridSpec(2, 2, height_ratios=[1, 1.2], hspace=0.35, wspace=0.3)

            # Top left - Full frame
            if show_full:
                ax_full = self.export_fig.add_subplot(gs[0, 0])
                if frame.image_data is not None:
                    ax_full.imshow(frame.image_data, cmap='viridis', aspect='auto')
                    if self.export_show_roi_box_var.get():
                        rect = Rectangle((effective_roi['x'], effective_roi['y']),
                                         effective_roi['width'], effective_roi['height'],
                                         linewidth=2, edgecolor='white', facecolor='none')
                        ax_full.add_patch(rect)
                ax_full.set_title(f"{camera_label} at {frame.timestamp} μs", fontsize=10)
                ax_full.set_xlabel("Pixel")
                ax_full.set_ylabel("Pixel")

            # Top right - ROI zoom
            if show_roi:
                ax_roi = self.export_fig.add_subplot(gs[0, 1])
                if frame.image_data is not None:
                    roi_image, roi_bounds = SpectrumAnalyzer.extract_roi_image(
                        frame.image_data, effective_roi, self.tilt_angle, self.flip_horizontal, expand_y=25
                    )
                    if roi_image is not None and roi_image.size > 0:
                        ...
                        ax_roi.axhline(roi_bounds[0], color='white', linestyle='--', linewidth=1, alpha=0.7)
                        ax_roi.axhline(roi_bounds[1], color='white', linestyle='--', linewidth=1, alpha=0.7)
                ax_roi.set_title(f"ROI at {frame.timestamp} μs", fontsize=10)

            # Bottom - Spectrum (spans both columns)
            if show_spectrum:
                ax_spec = self.export_fig.add_subplot(gs[1, :])
                coeffs = frame.get_effective_coeffs()

                # Get or extract spectrum
                spectrum = frame.spectrum
                if spectrum is None and frame.image_data is not None:
                    spectrum = SpectrumAnalyzer.extract_spectrum(
                        frame.image_data, effective_roi, self.tilt_angle, self.flip_horizontal
                    )

                spectrum_display = frame.spectrum_baseline_removed if frame.spectrum_baseline_removed is not None else spectrum

                if spectrum_display is not None:
                    if coeffs is not None:
                        wavelengths = np.array([SpectrumAnalyzer.pixel_to_wavelength(i, coeffs)
                                                for i in range(len(spectrum_display))])
                        ax_spec.plot(wavelengths, spectrum_display, 'g-', linewidth=1)
                        ax_spec.set_xlim(350, 900)
                        ax_spec.set_xlabel("Wavelength (nm)")
                    else:
                        ax_spec.plot(spectrum_display, 'g-', linewidth=1)
                        ax_spec.set_xlabel("Pixel")

                    ax_spec.set_ylabel("Intensity")
                    if self.export_gridlines_var.get():
                        ax_spec.grid(True, alpha=0.3)

                ax_spec.set_title(f"Spectra at {frame.timestamp} μs", fontsize=10)

            self.export_fig.suptitle(f"Preview: Frame {idx} ({len(frame_indices)} frame(s) selected)", fontsize=11)
            self.export_fig.tight_layout()
            self.export_canvas.draw_idle()
    ...
    def _export_selected_frames(self):
        """Export selected frames with chosen options."""
        if not self.frames:
        ...
        export_full = self.export_full_frame_var.get()
        export_roi = self.export_roi_var.get()
        export_spectrum = self.export_spectrum_var.get()
        show_roi_box = self.export_show_roi_box_var.get()
        show_gridlines = self.export_gridlines_var.get()

        if not (export_full or export_roi or export_spectrum):
        ...
        progress_label = ttk.Label(progress, text="Exporting frames...")
        progress_label.pack(pady=10)
        ...
        for i, idx in enumerate(frame_indices):
            progress_label.config(text=f"Exporting frame {idx} ({i + 1}/{len(frame_indices)})")
            progress_bar['value'] = (i / len(frame_indices)) * 100
            progress.update()

            frame = self.frames[idx]
            effective_roi = self.roi.copy()
            effective_roi['y'] += frame.roi_offset_y
            time_str = f"{frame.timestamp}us"

            # Export full frame - keep axes with pixel coordinates
            if export_full:
                fig, ax = plt.subplots(figsize=(8, 5), dpi=150)
                ax.imshow(frame.image_data, cmap='viridis', aspect='auto')
                if show_roi_box:
                    rect = Rectangle((effective_roi['x'], effective_roi['y']),
                                     effective_roi['width'], effective_roi['height'],
                                     linewidth=2, edgecolor='white', facecolor='none')
                    ax.add_patch(rect)
                ax.set_title(f"Frame {idx} - {frame.timestamp} us")
                fig.tight_layout()
                fig.savefig(os.path.join(folder, f"Frame{idx}_Full.png"),
                            dpi=150, bbox_inches='tight', facecolor='white')
                plt.close(fig)
                exported_count += 1

            # Export ROI - zoomed view with axes
            if export_roi:
                # Extract expanded ROI for context (like your example)
                x, y, w, h = effective_roi['x'], effective_roi['y'], effective_roi['width'], effective_roi['height']

                # Get a zoomed region around the ROI with some padding
                pad_x = 50
                pad_y = 50
                y_start = max(0, y - pad_y)
                y_end = min(frame.image_data.shape[0], y + h + pad_y)
                x_start = max(0, x - pad_x)
                x_end = min(frame.image_data.shape[1], x + w + pad_x)

                zoomed_region = frame.image_data[y_start:y_end, x_start:x_end]

                fig, ax = plt.subplots(figsize=(8, 5), dpi=150)

                # Show image with correct extent so axes show original pixel coordinates
                ax.imshow(zoomed_region, cmap='viridis', aspect='auto',
                ...
                ax.set_title(f"Frame {idx} - {frame.timestamp} us")
                fig.tight_layout()
                fig.savefig(os.path.join(folder, f"Frame{idx}_zoom.png"),
                            dpi=150, bbox_inches='tight', facecolor='white')
                plt.close(fig)
                exported_count += 1
            ...
            if export_spectrum:
                ...
                # Get spectrum - extract if not available
                spectrum = frame.spectrum
                if spectrum is None:
                    spectrum = SpectrumAnalyzer.extract_spectrum(
                        frame.image_data, effective_roi, self.tilt_angle, self.flip_horizontal
                ...
                if spectrum_display is not None and coeffs is not None:
                    wavelengths = np.array([SpectrumAnalyzer.pixel_to_wavelength(i, coeffs)
                                            for i in range(len(spectrum_display))])
                    ax.plot(wavelengths, spectrum_display, 'g-', linewidth=1)
                    ax.set_xlim(350, 900)
                    ax.set_xlabel("Wavelength (nm)")
                elif spectrum_display is not None:
                    ax.plot(spectrum_display, 'g-', linewidth=1)
                    ax.set_xlabel("Pixel")

                ax.set_title(f"Spectra at {frame.timestamp} μs")
                ax.set_ylabel("Intensity")
                if show_gridlines:
                    ax.grid(True, alpha=0.3)
                fig.tight_layout()
                fig.savefig(os.path.join(folder, f"Spectra_{time_str}.png"),
                            dpi=150, bbox_inches='tight', facecolor='white')
                plt.close(fig)
                exported_count += 1
        ...
        messagebox.showinfo("Export Complete",
                            f"Exported {exported_count} files for {len(frame_indices)} frame(s)\n\nLocation: {folder}")
    ...
    def _load_frames(self):
        ...
        self.export_slider.config(to=len(self.frames) - 1)
    ...
    # Export Methods
    # =========================================================================
    def _export_777nm_timeseries(self):
        """
        Export 777nm intensity values as a time series file.
        Format matches luminosity files for easy loading in Home Plotter.
        """
        if not self.frames:
            messagebox.showerror("Error", "No frames loaded")
            return

        # Check if we have any calibrated frames
        calibrated_count = sum(1 for f in self.frames if f.get_effective_coeffs() is not None)
        if calibrated_count == 0:
            messagebox.showerror("Error", "No calibrated frames. Run 'Interpolate & Process All Frames' first.")
            return
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            title="Export 777nm Time Series"
        )
        if not filepath:
            return

        # Parameters for 777nm extraction
        target_wavelength = 777.4  # nm (OI triplet)
        wavelength_window = 10.0  # nm window to search for peak (±5nm)

        # Extract 777nm intensity for each frame
        data_rows = []
        skipped = []

        print("\n" + "=" * 60)
        print("777nm EXPORT DIAGNOSTIC")
        print("=" * 60)
        ...
        for frame in self.frames:
            ...
            if coeffs is None:
                skipped.append((frame.index, "No calibration coefficients"))
                continue

            # ALWAYS re-extract spectrum with current settings for consistency
            effective_roi = self.roi.copy()
            effective_roi['y'] += frame.roi_offset_y

            raw_spectrum = SpectrumAnalyzer.extract_spectrum(
                frame.image_data, effective_roi, self.tilt_angle, self.flip_horizontal
            )

            if raw_spectrum is None or len(raw_spectrum) == 0:
                skipped.append((frame.index, "Failed to extract spectrum"))
                continue

            # Apply baseline removal with current settings (same as Review tab)
            if self.baseline_var.get():
                spectrum, _ = SpectrumAnalyzer.remove_baseline(raw_spectrum, self.baseline_degree_var.get())
            else:
                spectrum = raw_spectrum

            # Convert pixel indices to wavelengths
            pixels = np.arange(len(spectrum))
            wavelengths = np.array([SpectrumAnalyzer.pixel_to_wavelength(p, coeffs) for p in pixels])

            # Find pixels within the 777nm window
            mask = (wavelengths >= target_wavelength - wavelength_window / 2) & \
                   (wavelengths <= target_wavelength + wavelength_window / 2)

            if not np.any(mask):
                skipped.append((frame.index, f"777nm not in range ({wavelengths.min():.1f}-{wavelengths.max():.1f}nm)"))
                continue

            # Get wavelengths and intensities in window
            window_wavelengths = wavelengths[mask]
            window_intensities = spectrum[mask]

            # Find the peak within the window
            peak_idx_in_window = np.argmax(window_intensities)
            intensity_777 = window_intensities[peak_idx_in_window]
            actual_wavelength = window_wavelengths[peak_idx_in_window]

            # Debug output for first few frames and keyframes
            if frame.index < 3 or frame.is_keyframe:
                print(f"\nFrame {frame.index} (t={frame.timestamp} µs):")
                print(f"  Keyframe: {frame.is_keyframe}")
                print(f"  ROI offset: {frame.roi_offset_y}")
                print(f"  Wavelength range: {wavelengths.min():.1f} - {wavelengths.max():.1f} nm")
                print(f"  777nm window: {window_wavelengths.min():.1f} - {window_wavelengths.max():.1f} nm")
                print(f"  Peak found at: {actual_wavelength:.1f} nm")
                print(f"  Intensity: {intensity_777:.2f}")
                if frame.is_keyframe:
                    print(f"  Coeffs (own): {coeffs}")
                else:
                    print(f"  Coeffs (interpolated): {coeffs}")

            # Store time (µs) and intensity
            data_rows.append({
                'time': frame.timestamp,
                'intensity': intensity_777,
                'wavelength_found': actual_wavelength,
                'frame_idx': frame.index
            })

        print("\n" + "=" * 60)
        print(f"Exported: {len(data_rows)} frames")
        ...
        if not data_rows:
            messagebox.showerror("Error", "No valid 777nm data could be extracted")
            return

        # Write to file in luminosity format
        try:
            with open(filepath, 'w') as f:
                # Header comment
                f.write(f"# 777nm Spectroscopy Time Series\n")
                f.write(f"# Extracted from {len(data_rows)} frames\n")
                f.write(f"# Target wavelength: {target_wavelength} nm (window: ±{wavelength_window / 2} nm)\n")
                f.write(f"# Baseline removal: {self.baseline_var.get()} (degree={self.baseline_degree_var.get()})\n")
                f.write(f"# time_us\tintensity\twavelength_found\n")

                for row in data_rows:
                    f.write(f"{row['time']}\t{row['intensity']:.6f}\t{row['wavelength_found']:.2f}\n")

            # Report success
            msg = f"Exported 777nm time series:\n\n"
            msg += f"Frames exported: {len(data_rows)}\n"
            msg += f"Frames skipped: {len(skipped)}\n"
            msg += f"Time range: {data_rows[0]['time']} - {data_rows[-1]['time']} µs\n\n"
            msg += f"File: {filepath}\n\n"
            msg += "Check console for diagnostic output."
